...
def EXR2HDR(filename):
    # Because the input of puMSSSIM should be HDR format files and the output of HDRConvert is EXR format files,
...
def cal_puMSSSIM(args):
    ...
    for i in range(len(ref_imgs)):
        ...
        cmd = f'./HDR_Metrics "puMSSSIM" "{ref_img}" "{test_img}"'
        result = os.popen(cmd).read()
        # The result of HDR_Metrics is like:
        # Loading images...
        # Calculating...
        # The puMSSSIM is 1.000000
        puMSSSIM_avg = result.split("\n")[-2].split(" ")[-1]
        puMSSSIM.append(float(puMSSSIM_avg))
    return sum(puMSSSIM) / len(puMSSSIM)
...
if __name__ == '__main__':
    ...
    puMSSSIM = cal_puMSSSIM(args)
    print(f"The average puMSSSIM is {puMSSSIM:.6f}")
