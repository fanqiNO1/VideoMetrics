...
def EXR2HDR(filename):
    # Because the input of puPSNR should be HDR format files and the output of HDRConvert is EXR format files,
...
def cal_puPSNR(args):
    ...
    for i in range(len(ref_imgs)):
        ...
        cmd = f'./HDR_Metrics "puPSNR" "{ref_img}" "{test_img}"'
        result = os.popen(cmd).read()
        # The result of HDR_Metrics is like:
        # Loading images...
        # Calculating...
        # The puPSNR result is Inf
        puPSNR_avg = result.split("\n")[-2].split(" ")[-1]
        if puPSNR_avg == "Inf":
            puPSNR_avg = 100.0
        puPSNR.append(float(puPSNR_avg))
    return sum(puPSNR) / len(puPSNR)
...
if __name__ == '__main__':
    ...
    puPSNR = cal_puPSNR(args)
    print(f"The average puPSNR is {puPSNR:.6f}")
